	  ...
	  <div id="sidebar">							
			...
			<ul class="sidemenu">
				...
				<li><a href="Last.jsp"><font face="Tohama" color="#FFFFFF">SQL Last</font></a></li>
                  ...
                  <h2><P class=intro>The BETWEEN operator is used in a WHERE clause 
                  to select a range of data between two values.</P></h2>
                  <HR>

                  <H2>The BETWEEN Operator</H2>
                  <P>The BETWEEN operator selects a range of data between two 
                  values. The values can be numbers, text, or dates.</P>
                  <H3>SQL BETWEEN Syntax</H3>
                  <TABLE class=code cellSpacing=0 cellPadding=0 width="100%" 
                  border=0>
                    <TBODY>
                    <TR>
                      <TD>SELECT column_name(s)<BR>FROM table_name<BR>WHERE 
                        column_name<BR>BETWEEN value1 AND 
                  value2</TD></TR></TBODY></TABLE><BR>
                  <HR>
                  ...
                  <P>Now we want to select the persons with a last name 
                  alphabetically between "Hansen" and "Pettersen" from the table 
                  above.</P>
                  ...
                  border=0>
                    <TBODY>
                    <TR>
                      <TD>SELECT * FROM Persons<BR>WHERE LastName<BR>BETWEEN 
                        'Hansen' AND 'Pettersen'</TD></TR></TBODY></TABLE>
                  ...
                  <P><B>Note:</B> The BETWEEN operator is treated differently in 
                  different databases.</P>
                  <P>In some databases, persons with the LastName of "Hansen" or 
                  "Pettersen" will not be listed, because the BETWEEN operator 
                  only selects fields that are between and excluding the test 
                  values).</P>
                  <P>In other databases, persons with the LastName of "Hansen" 
                  or "Pettersen" will be listed, because the BETWEEN operator 
                  selects fields that are between and including the test 
                  values).</P>
                  <P>And in other databases, persons with the LastName of 
                  "Hansen" will be listed, but "Pettersen" will not be listed 
                  (like the example above), because the BETWEEN operator selects 
                  fields between the test values, including the first test value 
                  and excluding the last test value.</P>
                  <P>Therefore: Check how your database treats the BETWEEN 
                  operator.</P>
                  <HR>

                  <H2>Example 2</H2>
                  <P>To display the persons outside the range in the previous 
                  example, use NOT BETWEEN:</P>
                  <TABLE class=code cellSpacing=0 cellPadding=0 width="100%" 
                  border=0>
                    <TBODY>
                    <TR>
                      <TD>SELECT * FROM Persons<BR>WHERE LastName<BR>NOT 
                        BETWEEN 'Hansen' AND 'Pettersen'</TD></TR></TBODY></TABLE>
                  <P>The result-set will look like this:</P>
                  <TABLE class=reference cellSpacing=0 cellPadding=0 
                  width="100%" border=1>
                    <TBODY>
